...
export function serialize_config(cfg: lora_cfg_t): Uint8Array {
    ...
    tmp[2] = 0x08
    if (cfg.mode === 'relay') {
        tmp[3] = cfg.src_network
        tmp[4] = cfg.dst_network
        tmp[5] = 0x00
    }
    else {
        tmp[3] = cfg.address & 0xff00
        tmp[4] = cfg.address & 0x00ff
        tmp[5] = cfg.network
    }
    tmp[6] = air_rate_s[cfg.radio.air_rate] | mode_s[cfg.serial.mode] << 3 | baud_rate_s[cfg.serial.baud_rate] << 5
    tmp[7] = tx_power_s[cfg.radio.tx_power] | (cfg.serial.enable_mode_switch ? 1 : 0) << 2 | (cfg.serial.enable_RSSI_cmds ? 1 : 0) << 5 | frame_size_s[cfg.transport.frame_size] << 6
    tmp[8] = cfg.radio.channel
    tmp[9] = (cfg.transport.enable_LBT ? 1 : 0) << 4 | (cfg.mode === 'relay' ? 1 : 0) << 5 | (cfg.transport.fixed ? 1 : 0) << 6 | (cfg.transport.enable_RSSI ? 1 : 0) << 7
    return tmp
}
...
export function deserialize_config(array: Uint8Array, skip_sig_check = false): lora_cfg_t {
    ...
    cfg.mode = (((array[9] >> 5) & 1) === 0) ? 'endpoint' : 'relay'
    if (cfg.mode === 'endpoint') {
        cfg.address = array[3] << 8 + array[4]
        cfg.network = array[5]
    }
    else {
        cfg.src_network = array[3]
        cfg.dst_network = array[4]
    }
    cfg.radio.air_rate = air_rate_e[array[6] & 1]
    cfg.serial.mode = mode_e[((array[6] >> 3) & 3)]
    cfg.serial.baud_rate = baud_rate_e[((array[6] >> 5) & 7)]

    cfg.radio.tx_power = tx_power_e[array[7] & 3]
    cfg.serial.enable_mode_switch = !(((array[7] >> 2) & 1) === 0)
    cfg.serial.enable_RSSI_cmds = !(((array[7] >> 5) & 1) === 0)
    cfg.transport.frame_size = frame_size_e[(array[7] >> 6) & 3]

    cfg.radio.channel = array[8]

    cfg.transport.enable_LBT = !(((array[9] >> 4) & 1) === 0)
    cfg.transport.fixed = !(((array[9] >> 6) & 1) === 0)
    cfg.transport.enable_RSSI = !(((array[9] >> 7) & 1) === 0)
    return cfg
}
...
function test() {
    ...
    console.log(serialize_config(w));
    console.log(deserialize_config(serialize_config(w), false));
    console.log(
        serialize_config(deserialize_config(serialize_config(w), false)),
    );
}
